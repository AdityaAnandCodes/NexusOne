  ...
  try {
    ...
    if (session.user.email !== invitation.email) {
      return NextResponse.json(
        {
          error:
            "Email mismatch. Please sign in with the email the invitation was sent to.",
        },
        { status: 403 }
      );
    }
    ...
    console.log(
      `✅ Invitation accepted: ${session.user.email} joined company ${invitation.companyId} as ${invitation.role}`
    );
