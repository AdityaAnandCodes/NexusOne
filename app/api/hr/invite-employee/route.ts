...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, email, phone, skills, role, department, position } =
      await request.json();
    ...
    try {
      ...
      console.log(
        `✅ Invitation email sent to ${email} for ${name} at company ${company?.name}`
      );
      console.log(
        `📧 Generated email credentials: ${emailResult.email} / ${temporaryPassword}`
      );
      ...
      return NextResponse.json({
        ...
        invitationId: invitation._id,
        message:
          "Employee invitation created successfully, but email failed to send",
        emailSent: false,
